...
## T18 seam reconnaissance (2026-04-01)
- `src/store/run-store.ts` is the single runner-live-state seam and currently only stores `activeRunId`, `isStopping`, `progress`, and `status`; there is no explicit duplicate-stop guard, terminal message, or degraded/session-loss metadata.
- `src/routes/test-runner.tsx` is the only route subscribing to `runner.execution.started|progress|completed`; it already owns cancel UX/messages and is therefore the narrowest place to harden stop/cancel idempotency and empty/invalid-history messaging without widening app scope.
- `src/routes/api-tester.tsx` already keeps API-only execution independent from browser runtime and cleanly separates `preflight`, `transport`, and `assertion` failures; this route is the clearest proof surface for the T18 rule that browser failures must not block API features.
- `src/routes/web-recorder.tsx` plus `src/App.tsx`/`src/components/StatusBar.tsx` already contain the strongest degraded/recoverable/browser-unavailable messaging patterns, so T18 UI copy should reuse these patterns instead of inventing new wording styles.
- `src/services/api-tester-client.ts` and `src/services/web-recorder-client.ts` keep runtime branching inside typed clients; any T18 degraded/fallback hardening should stay in those client seams rather than adding route-level runtime branching.

## T18: reliability hardening, cancel idempotency, degraded isolation (2026-04-01)
- Idempotent stop/cancel semantics c?n neo ? src-tauri/src/state.rs tru?c; n?u ch? v� ? UI/service th� r?t d? c�n orphaned state khi completion/cancel v? ch?m ho?c l?p.
- V?i runner persistence, guard nh? nh?t nhung hi?u qu? l� ch?n terminal summary update khi completed_at d� c� v� ch?n duplicate per-case result b?ng INSERT ... SELECT ... WHERE NOT EXISTS theo kh�a (run_id, case_id, data_row_id).
- Oversized API preview an to�n hon khi truncate theo byte boundary b?ng String::from_utf8_lossy thay v� c?t theo chars, v� response th?c t? c� th? ch?a UTF-8 kh�ng ho�n ch?nh ho?c payload r?t l?n.
- Browser degraded/session-loss/deleted-reference messaging n�n gi? nguy�n ? BrowserAutomationService d? browser failures v?n r� nguy�n nh�n nhung kh�ng lan logic ch?n sang API-only flows.
- ? frontend runner seam, cancelled kh�ng n�n b? suy di?n l� v?n dang stopping; t�ch 	erminalMessage kh?i isStopping gi�p UI ph?n �nh trung th?c backend terminal semantics hon.


## T19: Smoke flows, MVP acceptance pass, and Week-6 browser viability gate report (2026-04-01)
- T19 gi? du?c scope nh? khi ch? orchestration l?i c�c seam d� c�: regression/source tests cho environment, API tester, recorder, runner, reliability, packaging v� m?t runtime smoke th?t cho browser replay.
- ��nh gi� MVP exit criteria c?n t�ch r� m?c b?ng ch?ng: seam-level proof d? cho c�c flow API/environment/runner, nhung ti�u ch� replay UI ch? du?c coi l� satisfied khi browser-replay-t14-smoke.ts tr? SMOKE_PASS.
- Vi?c t�ch task-T19-smoke-summary.txt kh?i task-T19-browser-gate.txt gi�p reviewer d?c nhanh verdict t?ng th? MVP v� quy?t d?nh Week-6 browser viability/fallback m� kh�ng ph?i suy lu?n t? log th�.

## F1 evidence backfill policy (2026-04-01)
- Khi backfill evidence muộn cho từng task, mỗi file evidence phải dựa trên command output mới chạy trong phiên hiện tại, không sao chép từ evidence cũ hay notepad.
- Nên tách rõ `PASS` và `BLOCKED` theo từng lệnh trong cùng task, nhất là với browser smoke/runtime prerequisite, để reviewer không hiểu nhầm source regression pass là runtime pass.
- Với task packaging/browser/Rust, nếu command thực tế không được chạy trong phiên backfill thì evidence phải ghi rõ "không claim" thay vì suy diễn từ việc toolchain có tồn tại.

 ## F1 packaging compliance learnings (2026-04-01)
...
pm test, 
pm run typecheck, 
pm run build) all pass in this workspace.
