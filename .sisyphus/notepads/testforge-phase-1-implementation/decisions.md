...
- Chosen orchestration seam: th�m duy nh?t tests/frontend/task-t19-smoke-report.ts + npm script test:t19:smoke nhu m?t wrapper m?ng ch?y l?i c�c regression/smoke d� t?n t?i, thay v� m? subsystem reporting m?i.
- Chosen evidence policy: d�ng hai file task-T19-smoke-summary.txt v� task-T19-browser-gate.txt du?i .sisyphus/evidence d? t�ch MVP exit evaluation kh?i Week-6 browser gate verdict nhung v?n gi? reviewer flow ng?n g?n.
- Chosen browser gate policy: gi? nguy�n semantics trung th?c t? T14 smoke (SMOKE_PASS | SMOKE_BLOCKED | SMOKE_FAIL) v� map sang PASS | BLOCKED | FAIL cho gate verdict; thi?u runtime prerequisite ph?i ra BLOCKED ch? kh�ng coi l� failure s?n ph?m hay pass gi?.
- Chosen MVP evaluation rule: c�c ti�u ch� environment/API/runner/packaging du?c d�nh gi� t? existing seam evidence, c�n replay UI b?t bu?c ph? thu?c smoke runtime th?t tru?c khi du?c d�nh d?u satisfied.
